...
function loadData() {
    ...
    var param = rawHash.slice(rawHash.indexOf('|') + 1);
...
function mainJs() {
    ...
    (0, _request2.default)(url, processFilms);
}
...
function onImageClick(e) {
    var filmId = e.target.getAttribute('data-id');
    var hash = '#details|' + filmId;
...
function appendEventListeners() {
    ...
    var imageList = document.querySelectorAll('.movie > img');
...
function renderDetails(id) {
    console.log(id);
    var url = 'http://api.myapifilms.com/imdb/idIMDB?idIMDB=' + id + '&token=5d0d20e7-5767-403f-b7a0-d070e43c8bba&format=json';
    (0, _request2.default)(url, processDetails);
}

function getWriters(film) {
    var writers = film.writers;

    var writerNames = writers.map(function (item) {
        return item.name;
    });
